...
Esta sección describe el procesamiento y limpieza de los datos de localización GPS de perros registrados en Movebank. Se incorporan variables clave para el análisis del movimiento: coordenadas del punto anterior (`lon_prev`, `lat_prev`), tiempo del punto anterior (`time_prev`), distancia entre puntos consecutivos (`dist_m`), diferencia de tiempo entre puntos (`time_diff_s`), velocidad de desplazamiento entre puntos consecutivos (`speed_ms`), dirección del movimiento (`angle`) y cambio de dirección entre pasos (`angle_change`). Finalmente, se eliminan registros con valores faltantes o no válidos.
...
Volviendo al cálculo de los ángulos; para cada punto de localización GPS, se calcula el **ángulo de desplazamiento** utilizando la función `atan2()`. Este ángulo mide la dirección del movimiento del perro entre dos puntos consecutivos en el plano de coordenadas, en radianes, donde:

* Un valor cercano a 0 indica movimiento hacia el este.
* Un valor cercano a $\frac{\pi}{2}$ (90°) indica movimiento hacia el norte.

Luego, se calcula el **cambio de ángulo** entre el movimiento actual y el anterior. Esto nos da el **cambio de dirección** o el giro entre dos puntos consecutivos:

* Un valor cercano a 0 indica que el perro sigue una trayectoria recta.
...
A continuación, se presenta un histograma de las velocidades calculadas para cada perro. Esto permite observar la distribución de velocidades y posibles diferencias entre individuos.
...
También se explora la relación entre el entorno y el comportamiento de movimiento. En el siguiente gráfico se muestra cómo varía el cambio de dirección (angle_change) según el uso del suelo, lo que podría indicar cambios de trayectoria asociados a distintos ambientes.
...
Finalmente, se compara la velocidad de movimiento en los distintos tipos de cobertura del suelo para cada perro. Esto puede ayudar a identificar si ciertos ambientes permiten o restringen el movimiento.
